...
namespace WinformControlLibraryExtension
{
    ...
    public class ThermometerExt : Control, IAnimationStaticTimer
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            float current_value_tmp = this.animation_current_value;
            if (current_value_tmp < this.MinValue)
            {
                current_value_tmp = this.MinValue;
            }
            if (current_value_tmp > this.MaxValue)
            {
                current_value_tmp = this.MaxValue;
            }
            if (this.animation_current_value < this.MaxValue)
            {
                float x = circle_rect.X + (circle_rect.Width - scale_width) / 2f;
                float y = scale_rect.Bottom - scale_height * ((current_value_tmp - this.MinValue) /sumValue );
