...
namespace WinformControlLibraryExtension
{
    ...
    public class NavigationBarExt : Control
    {
        ...
        public delegate void ItemOperatingEventHandler(object sender, ItemOperatingEventArgs e);
        public delegate void ItemOperatedEventHandler(object sender, ItemOperatedEventArgs e);

        private event ItemOperatingEventHandler itemDeselecting;
        /// <summary>
        /// 导航选项取消选中事件
        /// </summary>
        [Description("导航选项取消选中事件")]
        public event ItemOperatingEventHandler ItemDeselecting
        {
            add { this.itemDeselecting += value; }
            remove { this.itemDeselecting -= value; }
        }

        private event ItemOperatedEventHandler itemDeselected;
        /// <summary>
        /// 导航选项取消选中事件
        /// </summary>
        [Description("导航选项取消选中事件")]
        ...
        {
            add { this.itemDeselected += value; }
            remove { this.itemDeselected -= value; }
        }

        private event ItemOperatingEventHandler itemSelecting;
        /// <summary>
        /// 导航选项选中时事件
        /// </summary>
        ...
        {
            add { this.itemSelecting += value; }
            remove { this.itemSelecting -= value; }
        }

        private event ItemOperatedEventHandler itemSelected;
        /// <summary>
        /// 导航选项选中后事件
        /// </summary>
        ...
        private event ItemOperatedEventHandler selectedIndexChanged;
        /// <summary>
        /// 导航选项选中索引更改后事件
        /// </summary>
        [Description("导航选项选中索引更改后事件")]
        public event ItemOperatedEventHandler SelectedIndexChanged
        {
        ...
        public int SelectedIndex
        {
            ...
            set
            {
                ...
                ItemOperatingEventArgs deselecting_arg = new ItemOperatingEventArgs(this.FindItem(this.selectedIndex), this.selectedIndex);
                ...
                ItemOperatedEventArgs deselected_arg = new ItemOperatedEventArgs(this.FindItem(this.selectedIndex), this.selectedIndex);
                this.OnItemDeselected(deselected_arg);

            result:
                int tmp_index = this.selectedIndex;
                NavigationBarItem tmp_item = this.FindItem(this.selectedIndex);
                this.selectedIndex = value;
                this.selectedItem = this.FindItem(this.selectedIndex);

                ItemOperatingEventArgs selecting_arg = new ItemOperatingEventArgs(this.FindItem(this.selectedIndex), this.selectedIndex);
                this.OnItemSelecting(selecting_arg);
                if (selecting_arg.Cancel == false)
                {
                    ItemOperatedEventArgs selected_arg = new ItemOperatedEventArgs(this.FindItem(this.selectedIndex), this.selectedIndex);
                    this.OnItemSelected(selected_arg);

                    this.OnSelectedIndexChanged(new ItemOperatedEventArgs(this.SelectedItem, this.selectedIndex));
        ...
        protected override bool ProcessDialogKey(Keys keyData)
        {
            if (this.activatedState)
            {
                ...
                else if (keyData == Keys.Enter || keyData == Keys.Space)
                {
                    ...
                    {
                        this.SelectedIndex = this.activatedStateIndex;
                    }
                    return false;
                }
                #endregion
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            if (e.Button == System.Windows.Forms.MouseButtons.Left)
            {
                ...
                for (int i = 0; i < this.Items.Count; i++)
                {
                    ...
                    {
                        isfind = true;
                        this.currentMouseDownObject = this.Items[i];
                        goto result;
                    }
                }
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (e.Button == System.Windows.Forms.MouseButtons.Left)
            {
                ...
                for (int i = 0; i < this.Items.Count; i++)
                {
                    if (this.Items[i].BackRectF.Contains(point) && this.currentMouseDownObject == this.Items[i])
                    {
                        if (this.TabStop)
                        {
                            this.Select();
        ...
        #region 虚方法
        protected virtual void OnItemDeselecting(ItemOperatingEventArgs e)
        {
            if (this.itemDeselecting != null)
            {
                this.itemDeselecting(this, e);
            }
        }

        protected virtual void OnItemDeselected(ItemOperatedEventArgs e)
        {
        ...
        protected virtual void OnItemSelecting(ItemOperatingEventArgs e)
        {
            if (this.itemSelecting != null)
            {
                this.itemSelecting(this, e);
            }
        }

        protected virtual void OnItemSelected(ItemOperatedEventArgs e)
        {
        ...
        internal virtual void OnSelectedIndexChanged(ItemOperatedEventArgs e)
        {
        ...
        /// 查找指定选项的索引
        /// </summary>
        /// <param name="item">指定选项</param>
        /// <returns>找不到返回-1</returns>
        public int FindItemIndex(NavigationBarItem item)
        {
            if (this.Items != null)
            {
                for (int i = 0; i < this.Items.Count; i++)
        ...
        /// 查找指定选项的索引
        /// </summary>
        /// <param name="item">指定选项索引</param>
        /// <returns>找不到返回null</returns>
        public NavigationBarItem FindItem(int index)
        {
        ...
        /// 导航选项操作时事件参数
        /// </summary>
        [Description("导航选项操作时事件参数")]
        public class ItemOperatingEventArgs : CancelEventArgs
        {
            /// <summary>
            /// 导航选项
            /// </summary>
            ...
            /// <param name="itemIndex">导航选项索引</param>
            public ItemOperatingEventArgs(NavigationBarItem item, int itemIndex)
            {
                this.Cancel = false;
                this.Item = item;
        ...
        /// 导航选项操作后事件参数
        /// </summary>
        [Description("导航选项操作后事件参数")]
        public class ItemOperatedEventArgs : EventArgs
        {
            /// <summary>
            /// 导航选项
            /// </summary>
            ...
            /// <param name="item">导航选项</param>
            /// <param name="itemIndex">导航选项索引</param>
            public ItemOperatedEventArgs(NavigationBarItem item, int itemIndex)
            {
                this.Item = item;
                this.ItemIndex = itemIndex;
            }
